...
export const ExcelPreview = () => {
  ...
  if (!contextValue?.fileData) return null;
  ...
  return (
    <div className="fixed left-[2rem] top-[10rem] max-w-[60rem] max-h-[40rem] overflow-auto z-100">
      <div className="border-l border-b border-black bg-white/30">
        {contextValue.fileData.map((row, i) => {
          return (
            <div className="flex border-t border-black overflow-hidden" key={i}>
              {row.map((cell) => {
                return (
                  <div
